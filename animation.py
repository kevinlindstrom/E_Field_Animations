...
def makeAll():
    ...
    # P
    p = [w/2, h/2.5]
    my_canvas.create_circle(w/2, h/2.5, 6*thickness/4, fill="white")
    ...
    my_canvas.create_line(w/2, 9*h/10, w/2, p[1], width=thickness, arrowshape=arrow_tuple, fill="red", arrow=tk.LAST) # ry
    my_canvas.create_line(x, 9*h/10, w/2, 9*h/10, width=thickness, arrowshape=arrow_tuple, fill="red", arrow=tk.LAST) #rx
...
B=tk.Button(root,text="Make Plots",command=plotFields)
B.pack()
